...
namespace DFPRpt__elan_DFP_Video
{
    public class DFPRpt__elan_DFP_Video : DFPRpt_base, iDFPReportDownloader
    {
        public void define(ReportConfig config, string timespan)
        {
            ...
            string opt = getRunning2000x2000creativeIds(config);

            reportJob.reportQuery.statement.query = (opt == "0") ? "NORUN" : "where creative_id in (" + getRunning2000x2000creativeIds(config) + ") AND salesperson_id in (" + get_fina_UserIds(config) + ")";
